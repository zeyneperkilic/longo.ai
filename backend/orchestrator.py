...
SYSTEM_HEALTH = ("Adın Longo. Sağlık, supplement ve laboratuvar konularında yardımcı ol. "
                 "Zararsız sohbet cümlelerine ('naber', 'nasıl yani', 'anladım' vb.) doğal yanıt ver. "
                 "SADECE tamamen sağlık dışı konularda (film, teknoloji, gündem, spor vb.) kibarca reddet ve sağlığa yönlendir. "
                 ...
                 "STİL: Kısa, net, doğal ve sohbet akışını koru. "
...
SYSTEM_HEALTH_ENGLISH = ("Your name is Longo. Help with health, supplements and lab topics. "
                          "Respond naturally to harmless chat phrases ('how are you', 'I see', 'okay', etc.). "
                          "ONLY politely redirect completely off-topic subjects (movies, tech, news, sports, etc.) to health. "
                          ...
                          "STYLE: Short, clear, natural and maintain conversation flow. "
...
def build_single_session_prompt(session_tests: List[Dict[str, Any]], session_date: str, laboratory: str) -> List[Dict[str, str]]:
    ...
    system_prompt = (
        ...
        "Her test sonucunun KLİNİK ANLAMINI ÇOK DETAYLI AÇIKLA: "
        "1) Bu test NEDEN önemli? Vücutta hangi fonksiyonu gösteriyor? "
        "2) Yüksek/düşük değer HANGİ SAĞLIK SORUNLARINA işaret edebilir? (Somut örnekler ver) "
        "3) Bu değer kullanıcının GÜNLÜK HAYATINDA nasıl etkileyebilir? (Yorgunluk, enerji, bağışıklık vb.) "
        "4) Uzun vadede DİKKAT EDİLMESİ gereken riskler neler? "
        "5) Testler arasında bir İLİŞKİ var mı? (Örn: D vitamini düşük + Hemoglobin düşük = beslenme sorunu) "
...
def build_multiple_lab_prompt(tests_data: List[Dict[str, Any]], session_count: int, available_supplements: List[Dict[str, Any]] = None, user_profile: Dict[str, Any] = None, quiz_data: Dict[str, Any] = None) -> List[Dict[str, str]]:
    ...
    tests_info += f"ÖNEMLİ: Bu analizde {session_count} farklı test seansı bulunmaktadır!\n\n"
